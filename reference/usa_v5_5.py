"""
USA Tactical Opportunity Monitor v5.5
=====================================

Obiettivo
---------
Selezionare opportunità USA con orizzonte 3-6 mesi combinando:
- valutazione
...
Questa v5 automatizza solo ciò che può essere derivato in modo ragionevole dai dati.
...
STRATEGY_VERSION = "MASTER_USA_v1.5"
REPORT_NAME = "USA Tactical Value & Quality Monitor"
...
COMMISSION_PER_SIDE = float(os.getenv("COMMISSION_PER_SIDE", "18"))
...
MIN_MARKET_CAP = float(os.getenv("MIN_MARKET_CAP", "500000000"))
MIN_AVG_DOLLAR_VOLUME = float(os.getenv("MIN_AVG_DOLLAR_VOLUME", "10000000"))
MIN_AVG_DOLLAR_VOLUME_SOFT = float(os.getenv("MIN_AVG_DOLLAR_VOLUME_SOFT", "5000000"))

# Decision thresholds
MIN_SCORE_WATCH = int(os.getenv("MIN_SCORE_WATCH", "60"))
MIN_SCORE_BUY = int(os.getenv("MIN_SCORE_BUY", "74"))
...
MIN_NET_RR_TP1 = float(os.getenv("MIN_NET_RR_TP1", "1.50"))
...
DB_PATH = DATA_DIR / "trading_forward_test.db"
JSON_DIR = DATA_DIR / "snapshots"
...
DISCOVERY_LENSES = {
    # Non sono hard filter finali: sono lenti di discovery.
    # Il punteggio successivo decide la qualità reale.
    "QUALITY": {
...
def get_yfinance_details(ticker: str, spy_hist: Optional[pd.DataFrame] = None) -> Dict[str, Any]:
    try:
        ...
        if spy_hist is not None and not spy_hist.empty and "Close" in spy_hist.columns:
            ...
            spy1 = pct_return(spy_close, 21)
            spy3 = pct_return(spy_close, 63)
            spy6 = pct_return(spy_close, 126)
            rs_1m = None if perf1m is None or spy1 is None else perf1m - spy1
            rs_3m = None if perf3m is None or spy3 is None else perf3m - spy3
            rs_6m = None if perf6m is None or spy6 is None else perf6m - spy6
...
def market_regime_engine() -> Dict[str, Any]:
    spy = fetch_market_hist("SPY")
    qqq = fetch_market_hist("QQQ")
    vix = fetch_market_hist("^VIX")
    ...
    spy_s = state(spy)
    qqq_s = state(qqq)
    vix_close = safe_float(vix["Close"].dropna().iloc[-1]) if not vix.empty else None

    points = 0
    if spy_s["trend"] == "BULL":
        points += 2
    elif spy_s["trend"] == "BEAR":
        points -= 2

    if qqq_s["trend"] == "BULL":
        points += 1
    elif qqq_s["trend"] == "BEAR":
        points -= 1
...
def value_trap_engine(c: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if fcfg is not None and fcfg < -15:
        risk += 1
    ...
    return {"value_trap_risk": label, "value_trap_reasons": reasons, "value_trap_points": risk}
...
def score_valuation(c: Dict[str, Any]) -> Tuple[float, Dict[str, Any]]:
    ...
    if pe is not None:
        possible += 3
        points += 3 if 5 <= pe <= 15 else 2 if pe <= 22 else 1 if pe <= 30 else 0
    if fpe is not None:
        possible += 2
        points += 2 if 5 <= fpe <= 15 else 1 if fpe <= 22 else 0
    if peg is not None and peg > 0:
        possible += 2
        points += 2 if peg <= 1 else 1 if peg <= 1.5 else 0
    if pfcf is not None and pfcf > 0:
        possible += 2
        points += 2 if pfcf <= 12 else 1 if pfcf <= 18 else 0
    if ev is not None and ev > 0:
        possible += 2
        points += 2 if ev <= 10 else 1 if ev <= 14 else 0
    if fcfy is not None:
        possible += 1
        points += 1 if fcfy >= 4 else 0.5 if fcfy >= 2 else 0

    return point_score_ratio(points, possible, WEIGHTS["valuation"]), {"raw": points, "possible": possible}
...
def score_financial_strength(c: Dict[str, Any]) -> Tuple[float, Dict[str, Any]]:
    ...
    if cr is not None:
        possible += 2
        points += 2 if cr >= 1.5 else 1 if cr >= 1.0 else 0
    if de is not None:
        possible += 2
        points += 2 if de < 0.5 else 1.5 if de < 1 else 0.5 if de < 2 else 0
    if nd is not None:
        possible += 2
        points += 2 if nd < 1 else 1.5 if nd < 2 else 0.5 if nd < 3 else 0
    if ic is not None:
        possible += 2
        points += 2 if ic >= 8 else 1.5 if ic >= 4 else 0.5 if ic >= 2 else 0
    if alt is not None:
        possible += 1
        points += 1 if alt >= 3 else 0.5 if alt >= 1.8 else 0

    return point_score_ratio(points, possible, WEIGHTS["financial_strength"]), {"raw": points, "possible": possible}
...
def score_earnings_quality(c: Dict[str, Any]) -> Tuple[float, Dict[str, Any]]:
    ...
    return point_score_ratio(points, possible, WEIGHTS["earnings_quality"]), {"raw": points, "possible": possible}
...
def calculate_total_score(c: Dict[str, Any], rr_min: float, portfolio_sectors: Dict[str, int]) -> Dict[str, Any]:
    ...
    for name, fn in [
        ...
        if name in COMPONENT_MAX_POSSIBLE:
            ...
            coverage = possible / COMPONENT_MAX_POSSIBLE[name]
...
def decision_engine(c: Dict[str, Any], regime: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if rs_state == "WEAK":
        warnings_list.append("Forza relativa vs SPY debole")
    if c.get("has_data_anomalies"):
...
def passes_survival(c: Dict[str, Any]) -> Tuple[bool, List[str]]:
    ...
    if price is None or price <= MIN_PRICE:
        reasons.append("Price <= $5 o N/D")
    if cap is None or cap < MIN_MARKET_CAP:
        reasons.append("Market Cap < $500M o N/D")
    if adv is None or adv < MIN_AVG_DOLLAR_VOLUME_SOFT:
        reasons.append("Average Dollar Volume < $5M o N/D")
...
def build_candidate(row: pd.Series, spy_hist: pd.DataFrame, portfolio_sectors: Dict[str, int], regime: Dict[str, Any]) -> Dict[str, Any]:
    ticker = str(row.get("name", "")).strip().upper()
    yf_d = get_yfinance_details(ticker, spy_hist=spy_hist)
    ...
    c: Dict[str, Any] = {
        ...
        "currency": yf_d.get("currency") or "USD",
        # valuation
...
def build_candidates(df_tv: pd.DataFrame, regime: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    for i, (_, row) in enumerate(df_tv.iterrows(), start=1):
        ...
        c = build_candidate(row, spy_hist, portfolio_sectors, regime)
        candidates.append(c)
...
def operational_rank_key(c: Dict[str, Any]) -> Tuple[Any, ...]:
    ...
    return (
        ...
        1 if prebuy_engine(c).get("prebuy_eligible") else 0,
        prebuy_engine(c).get("prebuy_score", 0),
        g["gates_passed"],
...
def select_ranked(candidates: List[Dict[str, Any]]) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
    ...
    for c in eligible:
        ...
        c.update(prebuy_engine(c))
        c["display_state"] = display_state(c)
    ...
    for c in ordered:
        ...
        if sector_count.get(sector, 0) >= MAX_PER_SECTOR:
            continue
        if (
            (c.get("opportunity_score") or c.get("score") or 0) < MIN_SCORE_WATCH
            and c.get("decision") not in {"BUY_NOW", "BUY_LIMIT"}
            and not bool(c.get("prebuy_eligible"))
        ):
            continue
        selected.append(c)
...
def history_health() -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            "history_runs": runs,
            "history_candidates": snaps,
            "history_note": "Database presente ma privo di storico utile.",
        }
    except Exception as e:
        return {
...
def prebuy_engine(c: Dict[str, Any], rr_min: Optional[float] = None) -> Dict[str, Any]:
    """
    Readiness operativo 1-10.

    NON modifica Decision Engine, soglie BUY, R/R, sizing o trigger.
    Serve solo a ordinare e rendere visibili i titoli vicini all'acquisto.

    10/10 non forza un BUY: BUY_NOW / BUY_LIMIT restano determinati
    esclusivamente dal Decision Engine esistente.

    Componenti (max 2 punti ciascuna):
      - score/opportunity
      - R/R
      - prezzo rispetto alla Buy Zone
      - trigger
      - sicurezza operativa (dati, earnings, struttura, sizing)
    """
    rr_min = MIN_NET_RR_NORMAL if rr_min is None else rr_min
    g = gate_status(c, rr_min)
    gates = g["gate_status"]

    score = safe_float(c.get("opportunity_score") or c.get("score")) or 0.0
    rr1 = safe_float(c.get("net_rr_tp1"))
    rr2 = safe_float(c.get("net_rr_tp2"))
    price = safe_float(c.get("price"))
    buy_low = safe_float(c.get("buy_zone_low"))
    buy_high = safe_float(c.get("buy_zone_high"))
    atr = safe_float(c.get("atr"))
    trigger_state = str(c.get("trigger_state") or "").upper()

    # 1) Score: pieno se BUY threshold superata; parziale se marginale.
    if score >= MIN_SCORE_BUY:
        score_pts = 2.0
    elif score >= MIN_SCORE_BUY - SCORE_MARGINAL_GAP:
        score_pts = 1.5
    ...
    # 2) R/R: pieno solo se TP1 e TP2 superano i gate esistenti.
    rr1_ratio = (rr1 / MIN_NET_RR_TP1) if rr1 is not None and MIN_NET_RR_TP1 > 0 else 0.0
    rr2_ratio = (rr2 / rr_min) if rr2 is not None and rr_min > 0 else 0.0
    if rr1_ratio >= 1.0 and rr2_ratio >= 1.0:
        rr_pts = 2.0
    elif rr2_ratio >= 1.0 or (rr1_ratio >= 0.90 and rr2_ratio >= 0.90):
        rr_pts = 1.25
    elif rr1_ratio >= 0.75 and rr2_ratio >= 0.75:
        rr_pts = 0.5
    else:
        rr_pts = 0.0

    # 3) Prezzo: Buy Zone = massimo; entro 1 ATR = molto vicino.
    in_zone = bool(c.get("in_buy_zone"))
    above_max_buy = bool(c.get("above_max_buy"))
    if in_zone:
        price_pts = 2.0
    elif above_max_buy:
        price_pts = 0.0
    elif price is not None and buy_high is not None and atr and atr > 0 and 0 < price - buy_high <= atr:
        price_pts = 1.5
    elif price is not None and buy_low is not None and atr and atr > 0 and 0 < buy_low - price <= atr:
        price_pts = 1.25
    else:
        distance = abs(safe_float(c.get("distance_to_buy_zone_pct")) or 999.0)
        price_pts = 0.75 if distance <= 5.0 else 0.0

    # 4) Trigger: confermato = 2; WAITING = 1.5; altro = 0.
    if trigger_state == "CONFIRMED":
        trigger_pts = 2.0
    elif trigger_state in {"WAITING", "PENDING", "NEAR"}:
        trigger_pts = 1.5
    else:
        trigger_pts = 0.0

    # 5) Sicurezza operativa. DATA_REVIEW non viene nascosto:
    # può restare PRE-BUY, ma non diventa BUY.
    safety_core = all(gates[k] for k in ("earnings", "structure", "sizing"))
    if safety_core and gates["data"]:
        safety_pts = 2.0
    elif safety_core and c.get("data_review_required"):
        safety_pts = 1.5
    elif gates["earnings"] and gates["structure"]:
        safety_pts = 0.75
    else:
        safety_pts = 0.0

    raw = score_pts + rr_pts + price_pts + trigger_pts + safety_pts
    readiness = int(clamp(round(raw), 1, 10))

    hard_blockers = []
    if not gates["earnings"]:
        hard_blockers.append("earnings")
    if not gates["structure"]:
        hard_blockers.append("structure")
    if above_max_buy:
        hard_blockers.append("price_above_max_buy")
    ...
    # PRE-BUY è volutamente una categoria di attenzione, non di esecuzione.
    # Richiede readiness >= 6 e nessun blocker strutturale/earnings/prezzo.
    prebuy_eligible = (
        c.get("decision") not in {"BUY_NOW", "BUY_LIMIT", "AVOID", "DATA_INSUFFICIENT"}
        and readiness >= 6
    ...
    return {
        "prebuy_score": readiness,
        "prebuy_label": label,
        "prebuy_eligible": prebuy_eligible,
        "prebuy_missing": missing,
        "prebuy_hard_blockers": hard_blockers,
        "prebuy_components": {
            "score": score_pts,
            "rr": rr_pts,
            "price": price_pts,
...
def display_state(c: Dict[str, Any]) -> str:
    """
    Stato operativo semplificato mostrato in email:
    BUY NOW · BUY LIMIT · PRE-BUY · WAIT · AVOID.
    Gli stati diagnostici dettagliati restano salvati in operational_state.
    """
...
def action_needed_text(c: Dict[str, Any]) -> str:
    state = display_state(c)
    if state == "BUY NOW":
        return "COMPRA: setup confermato; verifica ordine e size"
    if state == "BUY LIMIT":
        return "INSERISCI LIMIT al livello indicato; non inseguire"
    if state == "PRE-BUY":
        pb = prebuy_engine(c)
        missing = ", ".join(pb.get("prebuy_missing") or []) or "conferma finale"
        return f"ALERT + controllo immediato; manca: {missing}"
    if state == "AVOID":
        return "NON COMPRARE"
    return "ASPETTA / monitora"
...
def build_action_board(selected: List[Dict[str, Any]], limit: int = 5) -> str:
    ...
    rows = ""
    for c in board:
        diagnostic_state = c.get("operational_state") or operational_state(c)
        visible_state = c.get("display_state") or display_state(c)
        pb = prebuy_engine(c)
        prebuy_txt = f"{pb['prebuy_score']}/10" if visible_state == "PRE-BUY" else "—"
        ...
        <tr>
          ...
          <td style='padding:7px'><b>{html_escape(visible_state)}</b><br><span style='font-size:10px;color:#6b7280'>{html_escape(diagnostic_state)}</span></td>
          <td style='padding:7px'><b>{prebuy_txt}</b></td>
    ...
    <div style='background:#ffffff;border:2px solid #2563eb;border-radius:10px;padding:14px;margin-top:14px'>
      ...
      <table style='width:100%;border-collapse:collapse;font-size:12px'>
        <thead style='background:#eff6ff'>
          <tr>
            <th>Ticker</th><th>Stato</th><th>PRE-BUY</th><th>Opportunity</th>
...
def generate_html(
    ...
    buy_now_count = sum(1 for c in selected if display_state(c) == "BUY NOW")
    limit_count = sum(1 for c in selected if display_state(c) == "BUY LIMIT")
    prebuy_count = sum(1 for c in selected if display_state(c) == "PRE-BUY")
    wait_count = sum(1 for c in selected if display_state(c) == "WAIT")
    avoid_count = sum(1 for c in selected if display_state(c) == "AVOID")
    new_count = sum(1 for c in selected if c.get("change_state") == "NEW")

    if buy_now_count:
        day_decision = f"{buy_now_count} BUY NOW - VERIFICARE ORDINE"
    elif limit_count:
        day_decision = f"{limit_count} BUY LIMIT PRONTO/I"
    elif prebuy_count:
        best_pb = max((prebuy_engine(c)["prebuy_score"] for c in selected if display_state(c) == "PRE-BUY"), default=0)
        day_decision = f"NESSUN BUY - {prebuy_count} PRE-BUY · MIGLIORE {best_pb}/10"
    elif selected:
        day_decision = "NESSUN BUY / PRE-BUY - ASPETTA"
    else:
    ...
    for c in material + repeats[:3]:
        ...
        <tr>
          ...
          <td style='padding:8px'>{html_escape(display_state(c))}</td>
          <td style='padding:8px'>{prebuy_engine(c).get('prebuy_score','—')}/10<br><span style='font-size:10px;color:#6b7280'>{html_escape(c.get('operational_state'))}</span></td>
        </tr>
        """
    ...
    for i, c in enumerate(selected[:TOP_N], 1):
        ...
        <div style='background:#fff;border:1px solid #e5e7eb;border-radius:12px;padding:18px;margin:16px 0'>
          ...
          <div style='margin-top:12px;font-size:12px;line-height:1.7'>
            ...
            <b>RS vs SPY:</b> {html_escape(c.get('rs_state'))} ·
    ...
    <body style='font-family:Arial,Helvetica,sans-serif;background:#f3f4f6;padding:20px;color:#111827'>
      <div style='max-width:1180px;margin:auto'>
        ...
        <div style='background:white;padding:14px;border-radius:10px;margin-top:14px'>
          <b>🌎 MARKET REGIME:</b> {html_escape(regime.get('regime'))} ·
          <b>VIX:</b> {fmt_num(regime.get('vix'),1)} ·
        ...
        <div style='background:#fff;padding:14px;border-radius:10px;margin-top:16px;font-size:12px'>
          ...
          <p><b>* Catalyst Proxy:</b> nella v5.1 è un proxy quantitativo. Non sostituisce la lettura di guidance, conference call o catalyst qualitativi verificati.</p>
...
def main() -> None:
    ...
    print(
        f"  {regime['regime']} | VIX={regime.get('vix')} | min Net R/R={regime['min_net_rr']} | "
        f"Portfolio Heat={fmt_pct(regime.get('portfolio_heat_pct'))}"
    )

    df_tv, removed_fields = run_tradingview_discovery()
    if df_tv.empty:
        subject = f"📊 USA Opportunities | {datetime.now().strftime('%d/%m/%Y')} | nessun candidato"
        html = (
    ...
    today = datetime.now().strftime("%d/%m/%Y")
    subject = (
        f"📊 USA Opportunities | {today} | {buy_now_count} BUY NOW · "
        f"{limit_count} BUY LIMIT · {prebuy_count} PRE-BUY"
        + (f" (max {best_pb}/10)" if prebuy_count else "")
        + f" · {wait_count} WAIT · {new_count} NEW"
    )
