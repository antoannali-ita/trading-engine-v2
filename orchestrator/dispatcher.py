...
TARGETS = {
    "CORE_USA": {"repo": "antoannali-ita/trading-engine-v2", "workflow": "master_scan.yml", "ref": "main", "inputs": {"market": "usa"}, "allowed_inputs": {"market", "request_id"}},
...
def dispatch_pending_requests(limit: int = 20) -> dict[str, int]:
    ...
    rows = db.table("manual_run_requests").select("*").eq("status", "REQUESTED").order("requested_at").limit(limit).execute().data or []
    dispatched = failed = 0
    for row in rows:
