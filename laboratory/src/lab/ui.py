...
STRATEGY_INFO = {
    ...
    "pead": {
        "label": "PEAD",
        "summary": "Post-Earnings Announcement Drift: cerca la continuazione dopo sorprese trimestrali usando dati point-in-time.",
        "signals": ["EPS Surprise", "Revenue Surprise", "Analyst Revisions", "Event Age", "Post-Earnings Reaction"],
    ...
    "event_driven_mean_reversion": {
        ...
        "signals": ["Event Return", "Volume Z-Score", "Day-1 Reaction", "Binary Event Filter"],
...
REGIME_LABELS = {
    "BULL_QUIET": "BULL QUIET", "BULL_VOLATILE": "BULL VOLATILE", "RANGE_NEUTRAL": "RANGE / NEUTRAL",
    "NEUTRAL": "NEUTRAL", "BEAR_HIGH_VOL": "BEAR / HIGH VOL", "BEAR": "BEAR",
}
...
def _sidebar_navigation() -> None:
    ...
    st.sidebar.page_link("pages/1_Signals.py", label="🎯  OPPORTUNITIES")
    st.sidebar.page_link("pages/5_Action_Center.py", label="⚡  ACTION CENTER")
    st.sidebar.page_link("pages/2_Portfolio.py", label="💼  PORTFOLIO")
    st.sidebar.markdown("---")
    st.sidebar.caption("ANALYSIS & RESEARCH")
...
def apply_theme() -> None:
    st.markdown(
        ...
        .status-good {background:rgba(34,197,94,.14);}.status-mid {background:rgba(234,179,8,.16);}.status-bad {background:rgba(239,68,68,.13);}.status-na {background:rgba(148,163,184,.16);}
        div[data-testid="stDataFrame"] {border:1px solid rgba(128,128,128,.13); border-radius:12px; overflow:hidden;}
        .candidate-title {font-size:1.02rem; font-weight:780; margin:0 0 .15rem 0; line-height:1.18;}.company-name {font-size:.76rem; opacity:.66; font-weight:500; margin-left:.2rem;}
        .candidate-state {font-size:.70rem; opacity:.62; margin:.05rem 0 .45rem 0; text-transform:uppercase; letter-spacing:.02em;}.candidate-detail {font-size:.74rem; opacity:.80; line-height:1.42; margin-top:.20rem;}
        .trigger-badge {display:inline-block; padding:3px 7px; border-radius:8px; font-size:.70rem; font-weight:800; line-height:1.1; white-space:normal;}.trigger-confirmed {background:rgba(34,197,94,.13);}.trigger-wait {background:rgba(234,179,8,.15);}.trigger-buy {background:rgba(59,130,246,.12);}
...
def candidate_title(ticker: Any, supplied_company: Any = None) -> str:
    t = "N/D" if ticker is None else str(ticker).strip().upper(); company = company_name(t, supplied_company)
...
def strategy_health(row_or_pf: Any, trades: Any = None, ret: Any = None) -> tuple[str, str]:
    if isinstance(row_or_pf, pd.Series):
        ...
        pf = _number(row_or_pf); trades_n = _number(trades) or 0; ret_n = _number(ret)
...
def render_strategy_card(name: str, results: Any, paper: pd.DataFrame | None = None) -> None:
    ...
    st.markdown(
        ...
        f'<div style="display:grid;grid-template-columns:repeat(3,1fr);gap:6px;margin-top:10px;font-size:.72rem"><div><b>Trades</b><br>{trades}</div><div><b>Avg PF</b><br>{fmt_num(avg_pf,2)}</div><div><b>Avg Return</b><br>{fmt_pct(avg_ret)}</div></div>'
        f'<div style="font-size:.71rem;line-height:1.36;opacity:.66;margin-top:9px"><b>Best for:</b> {html.escape(info["best_for"])}<br><b>Weak when:</b> {html.escape(info["weak_when"])}</div></div>',
        unsafe_allow_html=True,
    )
