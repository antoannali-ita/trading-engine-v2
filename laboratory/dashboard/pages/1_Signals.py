...
def _latest_per_ticker(frame: pd.DataFrame) -> pd.DataFrame:
    """Latest visible Core snapshot per market/ticker/horizon for decision cards.

    The full table keeps history; dashboard cards/summary use only the newest
    snapshot so repeated runs do not clone the same ticker on screen.
    """
    if frame.empty or "ticker" not in frame.columns:
        return frame.copy()
    out = frame.copy()
    if "created_at" in out.columns:
        out["_created_sort"] = pd.to_datetime(out["created_at"], errors="coerce", utc=True)
        out = out.sort_values("_created_sort", ascending=False, na_position="last")
    subset = [c for c in ["market", "ticker", "horizon"] if c in out.columns]
    if not subset:
        subset = ["ticker"]
    out = out.drop_duplicates(subset=subset, keep="first")
    return out.drop(columns=["_created_sort"], errors="ignore")
...
# Decision-facing widgets use one latest snapshot per ticker. Full Table below keeps history.
latest_view = _latest_per_ticker(view)
if "score_total" in latest_view:
    latest_view = latest_view.sort_values("score_total", ascending=False)

operational = {"BUY NOW", "BUY LIMIT", "PRE-BUY", "PRE_BUY", "PRE_BUY_HIGH", "SHADOW_BUY"}
state = latest_view.get("decision", pd.Series(index=latest_view.index, dtype=object)).fillna("").astype(str).str.upper()
status_state = latest_view.get("status", pd.Series(index=latest_view.index, dtype=object)).fillna("").astype(str).str.upper()
active_mask = state.isin(operational) | status_state.isin(operational)

k1, k2, k3, k4 = st.columns(4)
k1.metric("Latest Tickers", len(latest_view), help="One latest Core snapshot per ticker/market/horizon. Historical rows remain in Full Table.")
k2.metric("Operational", int(active_mask.sum()), help="BUY NOW / BUY LIMIT / PRE-BUY / PRE-BUY HIGH / SHADOW BUY on latest snapshots")
k3.metric("Average Score", fmt_score(latest_view["score_total"].mean()) if "score_total" in latest_view and latest_view["score_total"].notna().any() else "N/D")
k4.metric("Average Net R/R TP2", fmt_rr(latest_view["rr_net_tp2"].mean()) if "rr_net_tp2" in latest_view and latest_view["rr_net_tp2"].notna().any() else "N/D")

st.markdown("### Highlighted Candidates")
active = latest_view[active_mask].head(6)
if active.empty:
...
with left:
    if "score_total" in latest_view and latest_view["score_total"].notna().any():
        fig = px.histogram(latest_view, x="score_total", nbins=15, title="Score Distribution")
        fig.update_layout(height=330, margin=dict(l=10, r=10, t=42, b=10), xaxis_title="Score")
        st.plotly_chart(fig, use_container_width=True)
with right:
    if "status" in latest_view:
        counts = latest_view["status"].fillna("N/D").map(fmt_status).value_counts().rename_axis("Status").reset_index(name="Count")
...
with st.expander("Full Table · History", expanded=False):
...
st.caption("Cards, metrics and charts use only the latest Core snapshot per ticker. Full Table keeps historical snapshots. Intraday Current/Min/Max/% Oggi are cached market-data context and do not change the Core decision.")
