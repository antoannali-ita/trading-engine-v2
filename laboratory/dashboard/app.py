...
from lab.ui import apply_theme, company_name, fmt_money, fmt_pct, fmt_score, fmt_trigger, page_header
...
page_header(
    "Control Room",
    "Vista operativa del Laboratory: ladder, alert, paper positions e stato del motore. Il Core è un riferimento separato, non la fonte delle paper trade.",
)
...
try:
    ...
    st.error("Le tabelle operative del Lab non sono leggibili.")
    st.code(str(exc))
    st.stop()
...
c2.metric("Max posizione", fmt_money(MAX_POSITION_USD))
...
c6.metric("Paper aperte", len(open_paper))
st.caption(f"Policy Lab: max {MAX_NEW_BUYS} nuovi setup · preferenza {PREFERRED_ORDER_TYPE} · commissione USA simulata {fmt_money(USA_COMMISSION_USD)} per lato. Nessun ordine automatico.")

st.markdown("### Opportunity Ladder")
if lab_watch.empty:
    st.info("La watchlist Lab è vuota. Esegui Strategy Lab Daily Opportunity Feed per creare il primo stato operativo.")
else:
    ...
    if "trigger" in show:
        show["trigger"] = show["trigger"].map(fmt_trigger)
    ...
    st.dataframe(show[cols].head(30), use_container_width=True, hide_index=True)
    ...
    if not near.empty:
        ...
        for col, (_, row) in zip(cols_ui, near.iterrows()):
            with col:
                ...
                st.caption(f"{row.get('status', 'N/D')} · {row.get('strategy', 'N/D')}")
                st.metric("Score", fmt_score(row.get("score")))
                st.write(f"**Entry / Max Buy:** {fmt_money(row.get('entry'))} / {fmt_money(row.get('max_buy'))}")
                st.write(f"**Distanza entry:** {fmt_pct(row.get('distance_to_entry_pct'))}")
                st.write(f"**Trigger:** {fmt_trigger(row.get('trigger'))}")

st.markdown("### Paper Portfolio")
if open_paper.empty:
    st.info("Nessuna paper position aperta. Il Lab apre solo con PAPER_OPEN + trigger CONFIRMED.")
else:
    ...
    st.dataframe(paper[cols].head(20), use_container_width=True, hide_index=True)
...
with left:
    ...
    if signals.empty:
        ...
        st.dataframe(core[show_cols].head(15), use_container_width=True, hide_index=True)

with right:
    st.markdown("### Engine snapshot")
    if runs.empty:
        st.caption("Nessun run Core registrato.")
    else:
        latest = runs.iloc[0]
        st.metric("Ultimo run Core", str(latest.get("run_id", "N/D")))
    ...
    if not lab_watch.empty and "last_seen_at" in lab_watch:
        ...
        st.write(f"**Ultimo feed Lab:** {latest_lab if pd.notna(latest_lab) else 'N/D'}")

st.caption("Trading Lab 2.0 · operational tables del Laboratory in primo piano · Core separato · nessun ordine inviato automaticamente al broker.")
